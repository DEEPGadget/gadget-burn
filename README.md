...
| `-p sgemm_tf32` | FP32 storage + TF32 Tensor Core compute | `cublasGemmEx` + `CUBLAS_COMPUTE_32F_FAST_TF32` + `TENSOR_OP` |
...
`sgemm_tf32`는 [gpu-burn](https://github.com/wilicc/gpu-burn)의 `-tc` 옵션과 동일한 의도의 burn-in 모드입니다. A·B·C 매트릭스를 **FP32 storage 그대로** 두고, Tensor Core가 입력 mantissa를 TF32(19-bit)로 잘라 가속한 뒤 결과는 FP32로 누산합니다. FP32 메모리 대역폭 부하(매 iteration마다 풀 FP32 매트릭스 read/write) + TC compute 부하 + FP32 누산 회로가 동시에 활성화되어 회로 전체에 전류를 흘리는 효과가 있습니다. Ampere(sm_80) 이상에서만 TF32 가속이 동작하며, 그 이하는 일반 FP32로 폴백합니다.
...
이론치 TFLOPS 측정과 별개로 **하드웨어 안정성 burn-in** 용도에서는 silicon 전체에 최대 전력을 흘리는 것이 목적입니다. `-x` 플래그를 추가하면 [gpu-burn](https://github.com/wilicc/gpu-burn) 패턴을 모사한 세 가지 메커니즘이 동시에 활성화됩니다.

#### 1) 다중 C 버퍼 ring (memory write 트래픽 분산)

기본 모드는 단일 C 행렬에 반복 write — L2 cache 재사용이 많아 메모리 컨트롤러가 충분히 활성화되지 않습니다. `-x` 모드에서는 가용 VRAM을 채울 만큼 **다수의 C 슬롯을 ring buffer**로 할당하고, 매 GEMM iteration마다 다른 슬롯에 write합니다.

- L2 cache (96 MB on RTX 5090) << ring 전체 (수십 GB) → 모든 write가 DRAM까지 도달
- DRAM row buffer가 끊임없이 cycling → 메모리 컨트롤러 silicon 풀가동

#### 2) Compare 커널 (memory read + atomic 트래픽)

Ring 한 바퀴마다 모든 C 슬롯을 read해서 atomic으로 검증 카운터를 갱신하는 커널이 launch됩니다.

- 모든 슬롯 sequential read → DRAM read BW 활용
- `atomicAdd`로 thread 간 contention → L2 / NoC 추가 부담
- 부수 효과: 하드웨어 결함 발생 시 결과 mismatch 감지 가능

#### 3) Random 데이터 초기화 (silicon bit-toggle 확보)

기본 모드는 `cudaMemset(buf, 1, ...)`로 모든 byte를 `0x01`로 채웁니다. FP32 해석 시 `~2.4e-38` denormal 값이라 multiplier MAC에 입력되면 underflow flush로 처리되어 **bit toggle activity가 거의 0**이 됩니다 (특히 Tensor Core의 wide multiplier array가 영향 큼).

`-x` 모드는 xorshift32 PRNG로 GPU에서 직접 **random 0~10 (FP32/FP64) 또는 0~2 (FP16)** 데이터를 채웁니다. 매 곱셈마다 bit pattern이 크게 변동 → MAC 회로 풀 스위칭 → dynamic power 풀가동.

`-I memset`/`-I rand`로 강제 override해서 두 방식의 TDP 차이를 직접 비교 측정 가능합니다.

#### 행렬 크기 옵션 (`-X`)

cuBLAS는 M 값에 따라 다른 내부 커널을 선택하므로, 행렬 크기는 burn 효과에 직접 영향을 줍니다.
...
`-X` 미지정 시 `BURN_DEFAULT_M=32768` 사용. 더 작은 행렬로 비교 측정하려면 `-X 8192` 같이 명시.

#### 사용 예시

```bash
# 권장: TF32 TC + burn pressure (기본 M=32768)
./gadget_burn -p sgemm_tf32 -x

# FP32 shader burn (TC 미사용, 와트당 FLOP 비효율 → silicon 활성도 극대)
./gadget_burn -p sgemm -x

# gpu-burn -tc 정확 모사 (M=8192)
./gadget_burn -p sgemm_tf32 -x -X 8192

# 비교 측정: rand 데이터의 TDP 기여도 isolate
./gadget_burn -p sgemm_tf32 -x -I memset   # burn ring만, denormal 데이터
./gadget_burn -p sgemm_tf32    -I rand     # ring 없이, random 데이터
```

#### TDP vs TFLOPS 트레이드오프

burn 모드는 **이론치 TFLOPS를 일부 양보하는 대신 silicon 전체에 더 많은 전력을 흘립니다**. 측정 결과는 다음 표를 참고하세요.

| 모드 | TFLOPS Peak% | TDP (RTX 5090, 600W limit) |
|---|---|---|
| `-p sgemm_tf32` (기본) | 95-99% | 380-490 W |
| `-p sgemm_tf32 -x` | 80-95% | 580-600 W |
| `-p sgemm` (FP32 shader) | 70-75% | 600 W |
| `-p sgemm -x` | 70-75% | 600 W |

→ **이론치 측정**이 목적이면 `-x` 없이, **하드웨어 burn-in**이 목적이면 `-x` 포함.
...
| `-p <타입>` | `sgemm` / `dgemm` / `hgemm` / `hgemm_mix` / `sgemm_tf32` | `hgemm_mix` |
| `-m <값>` | 메모리 사용량 (`80%` 또는 `8192`) | `100%` |
| `-g <목록>` | 사용할 GPU ID (쉼표 구분) | 전체 |
| `-x` | **Burn pressure 모드** 활성화 (max TDP burn-in) | 비활성 |
| `-X <크기>` | burn 모드 행렬 크기 override (`8192`/`16384`/`32768`) | `32768` |
| `-I <모드>` | A,B 데이터 초기화: `memset` / `rand` | auto |
| `-l` | GPU 목록 출력 후 종료 | — |
| `-h` | 도움말 | — |
...
```bash
# 기본 실행 (전체 GPU, hgemm_mix, 100% VRAM, 1시간) — 이론치 TFLOPS 측정
./gadget_burn

# Burn-in 모드 (max TDP, 하드웨어 thermal/power 검증용)
./gadget_burn -x                            # 기본 sgemm_tf32 + burn pressure
./gadget_burn -p sgemm -x                   # FP32 shader burn (RTX 5090에서 600W)
./gadget_burn -p sgemm_tf32 -x -X 32768     # TF32 + 큰 행렬 (기본)
./gadget_burn -p sgemm_tf32 -x -X 8192      # gpu-burn 정확 모사 (작은 행렬)
./gadget_burn -p sgemm_tf32 -x -I memset    # rand vs memset 비교 측정용

# 이론 피크 TFLOPS 측정
./gadget_burn -p hgemm -m 80% -t 7200       # FP16 누산 Tensor Core
./gadget_burn -p sgemm_tf32                 # TF32 가속 SGEMM
./gadget_burn -p sgemm                      # FP32 SGEMM (TC 미사용)

# 운영 옵션
./gadget_burn -g 0,1 -i 4 -t 1800          # GPU 0,1, 스트림 4개, 30분
./gadget_burn -m 1%                         # 낮은 부하 테스트 (작은 GEMM)
